...
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -e angsd_sfs_d.err            # File to which STDERR will be written
#SBATCH -o angsd_sfs_d.out           # File to which STDOUT will be written
#SBATCH -J angsd_sfs_d           # Job name
...
POP=lorentzi
echo $POP
MININD=$(if [[ "$POP" == "blackchest" ]]; then echo 7; elif [[ "$POP" == "whitechest" ]]; then echo 8; elif [[ "$POP" == "lorentzi" ]]; then echo 5; elif [[ "$POP" == "whitesp" ]]; then echo 11; fi)
BAMLIST=$HOME_D/${POP}_bamlist_NR.txt
/home/eenbody/BI_software/angsd0.920_tar_install/angsd/angsd -b $BAMLIST -ref $REFGENOME -anc $REFGENOME -out Results/${POP}.${RUN}.ref -rf $REGIONS -P 20\
              -uniqueOnly 1 -remove_bads 1 -only_proper_pairs 0 -trim 0 \
              -minMapQ 20 -minQ 20 -minInd $MININD -doCounts 1 \
              -GL 1 -doSaf 1
#/home/eenbody/BI_software/angsd0.920_tar_install/angsd/misc/realSFS Results/${POP}.${RUN}.ref.saf.idx -P 20 > Results/${POP}.${RUN}.ref.sfs
